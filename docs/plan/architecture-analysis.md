...
> **opencode**：AnserFlow 自研的 Go 语言 AI Agent CLI 工具，在 Docker 沙箱中作为 Agent 运行时执行编码任务。支持 `read/write/bash/glob` 工具调用、thinking 模式和最大迭代次数控制。通过 `agents.runtime_config.opencode` 字段配置行为参数。
...
**Admin 端组织上下文**：admin 前端路由为扁平结构（`/admin/[locale]/agents`），但 API 需要 `org_id`。admin 通过 Sidebar 顶部组织选择器确定当前组织，以 URL query 或 Zustand store 传递：
...
# opencode CLI（AnserFlow Agent 运行时，原生 Go 二进制）
# 构建阶段需额外步骤编译安装，或从 Release 下载预编译版本
...
> 预估镜像大小约 120MB（含 Node.js + Python + Git + opencode 二进制）。
...
| **当前收口** | 本轮统一采用 `AutoMigrate + 备份 SQL`；`golang-migrate/migrate` 放入 Phase 2 独立任务 |
