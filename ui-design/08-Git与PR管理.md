...
- PR 列表刷新时保留当前筛选，不重置状态筛选。
- 手动创建 PR 弹窗打开后先展示模板预览；用户编辑描述后切换分支不覆盖已编辑内容。
- 新增 Git 凭证保存成功后，凭证列表插入新凭证并短暂高亮。
- 删除当前项目正在使用的凭证时，确认弹窗必须提示会影响 Git 校验、导入和 PR 创建。
- Git Issues 导入任务完成后，结果弹窗保留 imported / skipped 明细；点击 imported 项可直接打开对应 Issue。
