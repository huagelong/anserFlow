...
2. 如果用户只说“把需求文档转成 UI 设计师文档”，优先查找当前项目里明显承载需求内容的 Markdown 文件或目录，例如名称包含 `requirement`、`requirements`、`prd`、`product`、`spec`、`需求`、`产品` 的位置；如果存在多个候选且无法判断，先询问用户。
...
如果用户没有说清楚，但可以低风险推断，就先说明假设再执行；如果候选输入有多个，先询问用户，不要擅自选。
